...
void app_main(void)
{
	...
	// NO MATTER WHAT YOU NEED TO DO THESE THREE THINGS BEFORE USING THE DISPLAY !!! 
	...
	ESP_LOGI(TAG, "Preparing bitmaps: a black screen and a box");
	uint8_t blank[8192 / 8];
	...
	ESP_ERROR_CHECK(esp_lcd_panel_draw_bitmap(panel, 0, 0, 128, 64, blank));
	ESP_LOGI(TAG, "Displaying a box"); 
	ESP_ERROR_CHECK(esp_lcd_panel_draw_bitmap(panel, 0, 0, 64, 32, box));
